...
const StatCard = ({ label, value, sub, icon, accent = false, highlight }) => (
  <div className={`bg-[#1f1b1b] border rounded-xl px-4 py-4 flex items-center gap-3 transition-all duration-200 ${
    accent
      ...
      : "border-[#2e2726] hover:border-[#c45a45]/20"
  }`}>
    ...
    <div className="min-w-0">
      ...
      <p className={`text-lg font-bold leading-none truncate ${highlight || "text-white"}`}>{value}</p>
      {sub && <p className="text-[#9e9593] text-[10px] mt-0.5 truncate">{sub}</p>}
    </div>
  </div>
);
...
const ReferralCard = ({ profile }) => {
  ...
  return (
    <div className="bg-[#1f1b1b] border border-[#2e2726] rounded-2xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#2e2726]">
        <div>
          <h2 className="text-sm font-bold text-white flex items-center gap-2">
            <FaNetworkWired className="text-[#c45a45]" /> Affiliate Program
          </h2>
          <p className="text-[11px] text-[#9e9593] mt-0.5">Invite investors and earn commission on their active contracts.</p>
        </div>
        <a href="/referrals" className="text-[10px] font-bold text-[#c45a45] hover:text-[#d06a55] transition-colors underline underline-offset-2 shrink-0">
          View Full Page →
        </a>
      </div>
      <div className="p-5 space-y-5">
          ...
          {[
            ...
            <div key={s.label} className="bg-[#121010] border border-[#2e2726] rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[#9e9593] text-[10px] mt-0.5 uppercase tracking-wide truncate">{s.label}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="flex flex-col sm:flex-row gap-2.5">
          <div className="flex-1 flex items-center gap-2 bg-[#121010] border border-[#2e2726] rounded-xl px-3.5 py-2.5 min-w-0">
            <FaLink className="text-[#9e9593] text-[10px] shrink-0" />
            <span className="text-[11px] font-mono text-[#9e9593] truncate">{referralLink}</span>
          </div>
          <button onClick={handleCopy} className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all duration-200 shrink-0 ${
            copied
              ? "bg-emerald-500/15 border border-emerald-500/30 text-emerald-400"
              : "bg-[#c45a45] hover:bg-[#a64633] text-white shadow-md shadow-[#c45a45]/20"
          }`}>
            {copied ? <><FaCheckCircle className="text-[10px]" /> Copied!</> : <><FaCopy className="text-[10px]" /> Copy Link</>}
          </button>
        </div>
        <div className="grid grid-cols-3 gap-3 pt-1 border-t border-[#2e2726]">
          {[
            ...
            <div key={s.step} className={`space-y-0.5 pt-3 ${i > 0 ? "border-l border-[#2e2726] pl-3" : ""}`}>
              <p className="text-[#c45a45] font-black text-xs">{s.step}.</p>
              <p className="text-white font-semibold text-[11px]">{s.title}</p>
              <p className="text-[#9e9593] text-[10px] leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
...
const UserDashboard = () => {
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[#9e9593] text-xs">Loading dashboard…</p>
          </div>
        </div>
      </DashboardLayout>
    );
  ...
  return (
    <DashboardLayout>
      <div className="text-white space-y-5 pb-8 bg-[#171515] min-h-screen p-4 rounded-xl">
        ...
        <div className="relative bg-[#1f1b1b] border border-[#2e2726] rounded-2xl px-6 py-5 overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <p className="text-[#9e9593] text-xs uppercase tracking-widest mb-1">Welcome back</p>
            ...
            <div className="flex items-center gap-6 sm:border-l border-[#2e2726] sm:pl-6 shrink-0">
              <div>
                <p className="text-[#9e9593] text-[10px] uppercase tracking-widest mb-0.5">Wallet Balance</p>
              ...
              <div>
                <p className="text-[#9e9593] text-[10px] uppercase tracking-widest mb-0.5">Active Plans</p>
          ...
          <div className="lg:col-span-2 bg-[#1f1b1b] border border-[#2e2726] rounded-2xl p-5 flex flex-col">
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex-1 h-2 rounded-full bg-[#121010] overflow-hidden">
                <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${currentSignal}%`, background: "linear-gradient(90deg,#c45a45,#e07060)" }} />
              </div>
                ...
                <div key={mod.name} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs font-medium">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] px-1.5 py-0.5 rounded font-bold" style={{ backgroundColor: `${mod.color}18`, color: mod.color }}>{mod.status}</span>
                      <span className="text-white font-bold">{mod.pct}%</span>
                    </div>
                  </div>
                  <div className="w-full h-1.5 rounded-full bg-[#121010] overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700" style={{ width: `${mod.pct}%`, backgroundColor: mod.color }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-[#1f1b1b] border border-[#2e2726] rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#2e2726]">
            <div>
              <h2 className="text-sm font-bold text-white">📊 Investment Tiers</h2>
              <p className="text-[11px] text-[#9e9593] mt-0.5">Available plans matching your tier level</p>
            </div>
            <TierBadge tier={profile.tier || "none"} />
          </div>
                    ...
                    <tr key={plan.name} className="border-b border-[#2e2726] hover:bg-[#2e2726]/40 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-2.5">
                          <span className="w-8 h-8 rounded-lg bg-[#121010] border border-[#2e2726] flex items-center justify-center text-base">{plan.icon}</span>
                          <div>
                      ...
                      <td className="p-4 text-center">
                        <span className="px-2 py-0.5 rounded-md font-semibold text-[11px]" style={{ backgroundColor: `${plan.color}15`, color: plan.color }}>{plan.duration}</span>
                      </td>
                      ...
                      <td className="p-4 text-center">
                        {isActive ? (
                          ...
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-[#121010] text-[#9e9593] border border-[#2e2726]">
        ...
        <div className="bg-[#1f1b1b] border border-[#2e2726] rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#2e2726]">
            <div>
              ...
              <p className="text-[11px] text-[#9e9593] mt-0.5">Top performing capital accounts this cycle</p>
            </div>
            <span className="text-[10px] font-bold bg-[#121010] border border-[#2e2726] text-[#9e9593] px-2.5 py-0.5 rounded-md">Rankings Live</span>
          </div>
          {topLoading ? (
            <div className="flex items-center justify-center py-12 text-[#9e9593] text-xs">
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#c45a45] mr-2.5" />
              Syncing standings…
            </div>
          ) : topList.length === 0 ? (
            <div className="text-center py-12 text-[#9e9593] italic text-xs">No entries found.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                <thead>
                ...
                <tbody>
                  {topList.map(inv => {
                    const rankLabel = inv.rank === 1 ? "🥇" : inv.rank === 2 ? "🥈" : inv.rank === 3 ? "🥉" : `#${inv.rank}`;
                    return (
                      <tr key={inv.rank} className="border-b border-[#2e2726] hover:bg-[#2e2726]/40 transition-colors">
                        <td className="p-4 text-center">
                          <span className="inline-flex items-center justify-center w-7 h-7 rounded-full font-bold bg-[#121010] border border-[#2e2726] text-[#9e9593]">{rankLabel}</span>
                        </td>
                        <td className="p-4"><p className="font-semibold text-[#c45a45]">{inv.name}</p></td>
                        <td className="p-4 text-center"><TierBadge tier={inv.tier} /></td>
                        <td className="p-4 text-right font-bold text-emerald-400">${Number(inv.total_invested).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                        <td className="p-4 text-center">
                          <span className={`inline-block px-2 py-0.5 rounded-md text-[11px] font-bold ${
                            inv.active_plans > 0
                              ? "bg-[#c45a45]/15 text-white border border-[#c45a45]/25"
                              : "bg-[#121010] text-[#9e9593] border border-[#2e2726]"
                          }`}>{inv.active_plans} Plans</span>
                        </td>
                      </tr>
                    );
                  })}
