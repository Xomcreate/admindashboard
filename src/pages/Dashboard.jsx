...
// Updated graph nodes color array to perfectly match your branding palette
...
const StatCard = ({ label, value, color = "text-white", accent, icon }) => (
  <div className={`bg-[#0A0A0B] border border-white/6 p-6 rounded-xl transition-all duration-300 ${accent}`}>
    <div className="flex items-center justify-between mb-3">
      <h2 className="text-xs font-semibold text-white/50 uppercase tracking-wider">{label}</h2>
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#0A0A0B] border border-white/10 rounded-lg p-3 text-sm">
      ...
      <div className="text-white font-sans max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard label="Total Users"
            value={stats?.users ?? 0}
            color="text-white" accent="hover:border-white/20" icon="👥" />
          ...
          <StatCard label="Total Withdrawals"
            ...
            color="text-[#c45a45]" accent="hover:border-[#c45a45]/40" icon="💸" />
        ...
        {/* TIER LEGEND */}
        <div className="bg-[#0A0A0B] border border-white/6 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-white/50 uppercase tracking-wider mb-4">Investor Tier System</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-slate-400/10 border border-slate-400/25 rounded-lg p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-white/50 mt-0.5">1 – 2 investments</p>
                <p className="text-xs text-white/50">Entry-level investor</p>
              </div>
            </div>
            <div className="bg-yellow-500/10 border border-yellow-500/25 rounded-lg p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-white/50 mt-0.5">3 – 5 investments</p>
                <p className="text-xs text-white/50">Mid-tier investor</p>
              </div>
            </div>
            <div className="bg-violet-500/10 border border-violet-500/25 rounded-lg p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-white/50 mt-0.5">6+ investments</p>
                <p className="text-xs text-white/50">Elite investor status</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#0A0A0B] border border-white/6 rounded-xl p-6">
            ...
            <p className="text-xs text-white/50 mb-5">Total capital allocated per plan</p>
            {categoryData.length === 0 ? (
              <div className="flex items-center justify-center h-48 text-white/50 italic text-sm">No data yet</div>
            ) : (
          ...
          <div className="bg-[#0A0A0B] border border-white/6 rounded-xl p-6">
            ...
            <p className="text-xs text-white/50 mb-5">Share of total capital by plan</p>
            {categoryData.length === 0 ? (
              <div className="flex items-center justify-center h-48 text-white/50 italic text-sm">No data yet</div>
            ) : (
                ...
                <div className="flex flex-col gap-2 flex-1 min-w-0">
                  {categoryData.map((entry, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs min-w-0">
                      ...
                      <span className="text-white/50 truncate">{entry.category}</span>
        ...
        {monthlyData.length > 0 && (
          <div className="bg-[#0A0A0B] border border-white/6 rounded-xl p-6">
            ...
            <p className="text-xs text-white/50 mb-5">Total investments received per month</p>
        ...
        <div className="bg-[#0A0A0B] border border-white/6 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-white/6">
            ...
            <span className="text-xs bg-white/5 border border-white/10 text-white/50 px-3 py-1 rounded-full">
              Top {topList.length}
            </span>
          </div>
          ...
          {topLoading ? (
            ...
            <div className="text-center py-16 text-white/50 italic text-sm">No investors yet.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-white/2 text-white/50 uppercase text-xs font-semibold tracking-wider border-b border-white/6">
                ...
                <tbody>
                  {topList.map((inv) => {
                    const rankLabel =
                      ...
                      inv.rank === 3 ? "🥉" : `#${inv.rank}`;
                    return (
                      <tr key={inv.rank} className="border-b border-white/6 hover:bg-white/2 transition-colors">
                        <td className="p-4 text-center">
                          <span className="inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold bg-white/5 border border-white/10 text-white/50">
                            {rankLabel}
                          </span>
                        </td>
                        <td className="p-4">
                          <p className="font-semibold text-white">{inv.name}</p>
                          <p className="text-xs text-white/30 mt-0.5">{inv.email}</p>
                        </td>
                        ...
                        <td className="p-4 text-center text-white/50 font-semibold">
