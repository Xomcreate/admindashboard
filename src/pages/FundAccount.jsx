...
function UserFundAccount() {
  ...
  if (success) {
    return (
      <div className={`text-white max-w-lg mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center px-4 rounded-xl p-8 ${pageBg}`}>
        ...
        <button
          ...
          className="bg-[#a64633] hover:bg-[#c45a45] text-white px-8 py-3 rounded-lg font-semibold transition-all duration-200 shadow-lg hover:shadow-[#c45a45]/20"
        >
  ...
  return (
    <div className={`text-white max-w-3xl mx-auto space-y-8 px-4 py-4 min-h-screen ${pageBg}`}>
            ...
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border transition-all ${
                step > s.n
                  ...
                  : `bg-transparent ${borderCol} ${mutedText}`
              }`}>
                {step > s.n ? "✓" : s.n}
              </div>
              <span className={`text-xs font-medium hidden sm:block ${step >= s.n ? "text-white" : mutedText}`}>
                {s.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`flex-1 h-px mx-3 transition-all ${step > s.n ? "bg-emerald-500" : borderCol}`} />
            )}
          </div>
        ))}
      </div>
            ...
            {CRYPTO_OPTIONS.map((c) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className={`${mutedText} text-xs`}>{c.network}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      {step === 2 && coin && (
        <div className="space-y-5">
          <button onClick={() => setStep(1)} className={`text-xs ${mutedText} hover:text-white flex items-center gap-1 transition-colors`}>
            ← Back
          </button>
          ...
          <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${coin.bg} ${coin.border}`}>
            ...
            <div>
              ...
              <p className={`text-xs ${mutedText}`}>{coin.network}</p>
            </div>
          </div>

          {/* Wallet Address */}
          <div className={`${cardBg} border ${borderCol} rounded-xl p-5 space-y-3`}>
            <p className={`text-xs font-semibold ${mutedText} uppercase tracking-wider`}>
              Send {coin.symbol} to this address
            </p>
            <div className={`flex items-center gap-3 ${inputBg} border ${borderCol} rounded-lg px-4 py-3`}>
          ...
          {/* Amount input */}
          <div className={`${cardBg} border ${borderCol} rounded-xl p-5 space-y-4`}>
            <p className={`text-xs font-semibold ${mutedText} uppercase tracking-wider`}>Enter deposit amount (USD)</p>
            <div className="relative">
              <span className={`absolute left-4 top-1/2 -translate-y-1/2 ${mutedText} font-semibold`}>$</span>
              <input
                type="number"
                placeholder="0.00"
                min="500"
                step="0.01"
                className={`w-full ${inputBg} border pl-8 pr-4 py-3 rounded-lg text-white placeholder-[#9e9593] focus:outline-none transition-colors ${
                  amountErr ? "border-red-500" : `${borderCol} focus:border-[#c45a45]`
                }`}
                value={amount}
            ...
            <p className={`text-xs ${mutedText}`}>Minimum deposit: <span className="text-white font-semibold">$500</span></p>
          </div>

          <button
            onClick={() => { const err = validateAmount(amount); if (err) { setAmountErr(err); return; } setStep(3); }}
            disabled={!amount}
            className="w-full bg-[#a64633] hover:bg-[#c45a45] disabled:opacity-40 disabled:cursor-not-allowed text-white py-3 rounded-lg font-semibold transition-colors shadow-lg"
          >
            Continue →
          </button>
        </div>
      ...
      {step === 3 && coin && (
        <form onSubmit={handleSubmit} className="space-y-5">
          <button type="button" onClick={() => setStep(2)} className={`text-xs ${mutedText} hover:text-white flex items-center gap-1 transition-colors`}>
            ← Back
          </button>

          {/* Summary */}
          <div className={`${cardBg} border ${borderCol} rounded-xl p-5 space-y-3`}>
            <p className={`text-xs font-semibold ${mutedText} uppercase tracking-wider`}>Deposit Summary</p>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className={`${primaryOrange} font-bold`}>${parseFloat(amount).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className={mutedText}>Network</span>
              <span className="text-white">{coin.network}</span>
            </div>
            <div className={`h-px ${borderCol}`} />
          ...
          {/* Proof upload */}
          <div className={`${cardBg} border ${borderCol} rounded-xl p-5 space-y-3`}>
            <p className={`text-xs font-semibold ${mutedText} uppercase tracking-wider`}>Upload Payment Proof</p>
            <p className={`text-xs ${mutedText}`}>
              Attach a screenshot or photo of your transaction confirmation.
            </p>
            <label className={`flex flex-col items-center justify-center gap-3 ${inputBg} border-2 border-dashed ${borderCol} hover:border-[#c45a45]/50 rounded-xl p-8 cursor-pointer transition-colors`}>
              <span className="text-3xl">{proof ? "✓" : "📎"}</span>
              <span className={`text-sm ${mutedText}`}>
                {proof ? proof.name : "Click to upload image"}
              </span>
              <span className={`text-xs ${mutedText}`}>PNG, JPG, JPEG — Max 5MB</span>
              <input
                type="file"
                accept="image/*"
                className="hidden"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3 rounded-lg font-semibold transition-colors shadow-lg"
          >
...
function AdminFundAccount() {
  ...
  const [actionLoading, setActionLoading] = useState(null); // deposit id being actioned
  ...
  return (
    <div className="text-white max-w-4xl mx-auto px-4 py-4 space-y-6 min-h-screen bg-[#171515]">

      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold tracking-wide">Deposit Management</h1>
        <p className={`${mutedText} text-sm mt-1`}>
          Review, approve, or decline user deposit requests.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className={`${cardBg} border ${borderCol} rounded-xl p-4`}>
            <p className={`${mutedText} text-xs mb-1`}>{s.label}</p>
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium capitalize transition-colors border-b-2 -mb-px ${
              filter === t
                ...
                : `${mutedText} border-transparent hover:text-white`
            }`}
          >
            {t}{t !== "all" && ` (${counts[t]})`}
          </button>
      ...
      {/* Deposit List */}
      {loading ? (
        <div className={`${mutedText} text-sm text-center py-16`}>Loading deposits…</div>
      ) : filtered.length === 0 ? (
        <div className={`${mutedText} text-sm text-center py-16`}>No deposits found.</div>
      ) : (
        <div className="space-y-4">
          {filtered.map((dep) => (
            <div
              key={dep.id}
              className={`${cardBg} border ${borderCol} rounded-xl overflow-hidden ${
                dep.status === "pending"
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 pt-4 pb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`${mutedText} text-xs`}>{dep.email ?? "—"}</p>
                  </div>
                </div>
              ...
              {/* Meta grid */}
              <div className={`grid grid-cols-2 sm:grid-cols-4 gap-3 px-5 pb-4 border-b ${borderCol}`}>
                {[
                  ...
                  <div key={m.label}>
                    <p className={`${mutedText} text-xs mb-0.5`}>{m.label}</p>
              ...
              <div className="px-5 py-4">
                <p className={`${mutedText} text-xs font-semibold uppercase tracking-wider mb-3`}>Payment proof</p>
                {dep.payment_proof ? (
                  ...
                  <div className={`${inputBg} border ${borderCol} rounded-lg px-4 py-6 text-center`}>
                    <p className={`${mutedText} text-xs`}>No proof uploaded</p>
                  </div>
                )}
              </div>
